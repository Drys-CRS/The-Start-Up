...
export async function POST(req: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // "balance" payment is the second instalment mid-build — don't advance the stage.
    // Every other successful payment (deposit or full) starts the build.
    if (scopeLockId && paymentType !== "balance") {
      try {
        await setSimpleColumn(SCOPE_BOARD_ID, scopeLockId, SCOPE.stage, "Deposit Paid");
      } catch (e) {
